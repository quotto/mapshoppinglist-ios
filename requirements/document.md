...
- **ブランドパレット（Android/iOS統一）**  
  ...
  - バリエーション: #1B7CB0（ライトモード補助色）、#1B6491（ダークモード補助色）  
  - アクセント: #EA5F52（主要アクション/FAB/警告）
  - セカンダリ: #18324A（補助UI要素）
  - セカンダリコンテナ: #D1E4F1（カード背景・情報表示）
  - ベース背景: #F6FAFD（ライトテーマ） / #0F1C2B（ダークテーマ）
  - オンプライマリ: #F6F4F3（プライマリカラー上のテキスト）
  - オンサーフェス: #0F1C2B（ライト）/ #F6F4F3（ダーク）
  - アウトライン: #A9BCCC（境界線）
  - ニュートラルボーダー: #D5E3EE（薄い境界線）
  - 成功: #3CB371（完了状態・ポジティブメッセージ）
  - エラー: #E53935（エラーメッセージ・削除アクション）
  
- **iOS実装**
  - `MapShoppingList/App/Theme/AppColors.swift`: ブランドカラー定義
  - `MapShoppingList/App/Theme/AppTheme.swift`: SwiftUI用カラー拡張（`.appPrimary`等）
  - `Assets.xcassets/AccentColor.colorset`: アクセントカラー（ライト/ダーク対応）
