...
- **取得タイミング**
    - 初回起動時にS1でガイダンスカードを表示し、「通知を許可すると地点付近でリマインドが届く」旨を案内した上で `UNUserNotificationCenter` の許可をリクエスト。
    - 位置情報はS1ガイダンスおよびS3/S4の利用開始前に「ジオフェンス通知を利用するため常に許可が必要」と説明し、`requestAlwaysAuthorization()` を実行。使用中許可のみの場合は設定アプリへの誘導を行う。
- **バックグラウンド起動**
